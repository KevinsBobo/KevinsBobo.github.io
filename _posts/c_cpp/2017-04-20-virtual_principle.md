...
3. 在父类数据成员中增加一个指向函数指针数组的指针```m_vfPtr```，子类不能再声明与父类同名的```m_vfPtr```数据成员，（因为这样虽然覆盖了父类的同名数据成员，但是父类的数据成员还是会出现在子类内存中，在使用时就会出现问题）
...
- 然后单步进入父类析构，查看内存：
    ...
    ![](assets/img/virtual/0x51_vftab_pit_vfptr_base.png)
